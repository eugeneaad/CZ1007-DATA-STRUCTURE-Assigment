...
void moveOddItemsToBack(LinkedList *ll)
{
	...
	int index = 0;									// Declare a index to keep track of the pointer
	int size;										// Declare a integer to store the size of the linklist
	
	cur = ll->head;									// Let the temporary ListNode pointer cur = head pointer of the linklist

	if (cur == NULL)								// Check if the head pointer is empty
		return 0;									

	size = ll->size;								// Get the size of the linklist

	while (size!=0)									// Create a while loop to check all the linklist item by decrement of the size
	{
		
		if ((cur->item) % 2 == 1)					// Check if the item is odd num
		{
			insertNode(ll, ll->size, cur->item);	// If yes, then insert the value at the back 
			cur = cur->next;						// Move to the next ListNode first before remove the current ListNode
			removeNode(ll, index);					// Remove the current ListNode 
													// Noted there is no increment of the index since the current ListNode is move to the back
		}
		else
		{
			cur = cur->next;						// if no, then move to the next ListNode
			index++;								// increse the index since the move to the next ListNode 
		}
